...
print("For example: 'dashcam1fast.mp4'")
...
# 1 video out of 5 samples crashes early on due to multiple detections over exact threshold scores?
...
while True:
    ...
    # test on 2 images
    # test1 = cv2.imread('benGranTurismo1.jpg')  # 'imread' is a cv2 function!
    ...
    # test2 = cv2.imread('benGranTurismo2.jpg')  # 'imread' is a cv2 function!
    # height, width, _ = test2.shape  # extract the height and width dimensions from the image.
    ...
    if key == 27: # required to break the while loop (27 is the ESC key in ASCII)
        break
    # cv2.waitKey(3000)  # this is the delay function (1000 seems to be a second?)
    # cv2.imshow('Image', test2)  # first test image is replaced by the second image in the same window.
    # cv2.waitKey(3000)
camFootage.release()
